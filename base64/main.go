...
func encode(reader *bufio.Reader) {
	...
	for grp := 0; grp < (bytesLen+2)/3; grp++ {
		start := grp * 3
		if start >= bytesLen {break}
		
		b1 := bytes[start]
		var b2, b3 byte = 0, 0
		if start+1 < bytesLen {b2 = bytes[start+1]}
		if start+2 < bytesLen {b3 = bytes[start+2]}
		...
		Res += string(c1) + string(c2) + string(c3) + string(c4)
